...
public class PartitaQuattro extends Partita implements Runnable {

	private GestioneDatabase gestDatabase = null;
	private ArrayList<Giocatore> classifica = null;
	private	final int numGiocatori = 4;
	private	final int punteggio1 = 24;
	private	final int punteggio2 = 6;
	...
	public ArrayList<Giocatore> istance4(){
	...
	public void run() {
		...
		if(inserisciGiocatori()){
			...
			System.out.println("VALORE DI ASSEGNATERRITORI IN PARTITA4: "+result);
					...
					while(giocatori.size()>1){
						...
						do{
							...
							if(turno>1){
								...
								user = it.next();
								System.out.println("P4,+MOSSA ---> "+result);
	...
	private boolean inserisciGiocatori(){
		Giocatore user = null;
		giocatori = istance4();
		Iterator<Giocatore> it = new Prepartita().listaUtenti4.iterator();
		...
		it = new Prepartita().listaUtenti4.iterator();
		...
		synchronized(lock){
			for (i = 1; i < 4; i++){
				new Prepartita().listaUtenti4.remove(0);
			}
		}
		it = new Prepartita().listaUtenti4.iterator();
