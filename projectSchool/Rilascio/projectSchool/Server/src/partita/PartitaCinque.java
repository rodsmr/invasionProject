...
public class PartitaCinque extends Partita implements Runnable {

	private GestioneDatabase gestDatabase = null;
	private ArrayList<Giocatore> classifica = null;
	private	final int numGiocatori = 5;
	private	final int punteggio1 = 32;
	private	final int punteggio2 = 8;
	...
	public ArrayList<Giocatore> istance5(){
	...
	public void run() {
		...
		if(inserisciGiocatori()){
			...
			System.out.println("VALORE DI ASSEGNATERRITORI IN PARTITA5: "+result);
					...
					while(giocatori.size()>1){
						...
						do{
							...
							if(turno>1){
								...
								user = it.next();
								System.out.println("P5,+MOSSA ---> "+result);
	...
	private boolean inserisciGiocatori(){
		Giocatore user = null;
		giocatori = istance5();
		Iterator<Giocatore> it = new Prepartita().listaUtenti5.iterator();
		...
		it = new Prepartita().listaUtenti5.iterator();
		...
		synchronized(lock){
			for (i = 1; i < 4; i++){
				new Prepartita().listaUtenti5.remove(0);
			}
		}
		it = new Prepartita().listaUtenti5.iterator();
